...
- fs/io/mem: A collection of local memory based filesystems
	- simple: A memory filesystem that requires only ASCII based file paths, supports RO Pearson hasing
- fs/io/os: A filesystem wrapper based around the "os" package

## Examples

The most complete examples will be in the GoDoc. But here is an excerpt for a few use cases.
...
err := Merge(
	optimized, 
	somePkg.Embeded, 
	"/js/", // Puts the content into a sub-directory
	WithTransform(
...
- Pulls from a groupcache first
- Tries a disk cache second
...
// Read our file out. Since this is our first read of this file, it will
// come from Azure Blob storage and back fill our other caches.
...
This package is fairly new (2021) and I should point out that there is a great package for filesystem abstractions [Afero](https://github.com/spf13/afero).  While I've never used it, spf13 is the author of several great packages and it looks like it has great support for several different filesystem types.

So why gopherfs?  When I started writing this I was simply interested in trying to take advantage of io/fs.  I saw Afero after I had written a couple of filesystems and it did not have io/fs support and was geared towards its own method of abstraction that was built long before io/fs was a twinkle in the Go authors' eyes. 
